package DAO;

import java.util.Random;

public class AES {
    //factors for calculation of 
    private static int factor, factork, factorr;
    private static byte[][] w;

    
                        //array of hexa values
                    private static int[] sbox = {  };

    
    
                     private static int[] inv_sbox = {  };

    
    
            private static int Rcon[] = {  };

    
    
    //OR FUNCTION TAKES BYTE AS INPUT
    private static byte[] xor_func(byte[] a, byte[] b) {
            byte[] out = new byte[a.length];
            for (int i = 0; i < a.length; i++) {
                    out[i] = (byte) (a[i] ^ b[i]);
            }
            return out;

    }

    
    
    
    //GENERATION OF SUBKEY
    private static byte[][] generateSubkeys(byte[] key) {
            byte[][] tmp = new byte[factor * (factorr + 1)][4];

            int i = 0;
            while (i < factork) {

                    tmp[i][0] = key[i * 4];
                    tmp[i][1] = key[i * 4 + 1];
                    tmp[i][2] = key[i * 4 + 2];
                    tmp[i][3] = key[i * 4 + 3];
                    i++;
            }
            i = factork;
            while (i < factor * (factorr + 1)) {
                    byte[] temp = new byte[4];
                    for(int k = 0;k<4;k++)
                            temp[k] = tmp[i-1][k];
                    if (i % factork == 0) {
                            temp = SubWord(rotateWord(temp));
                            temp[0] = (byte) (temp[0] ^ (Rcon[i / factork] & 0xff));
                    } else if (factork > 6 && i % factork == 4) {
                            temp = SubWord(temp);
                    }
                    tmp[i] = xor_func(tmp[i - factork], temp);
                    i++;
            }

            return tmp;
    }

    
    
    //WORD LENGTH
    private static byte[] SubWord(byte[] in) {
            byte[] tmp = new byte[in.length];

            for (int i = 0; i < tmp.length; i++)
                    tmp[i] = (byte) (sbox[in[i] & 0x000000ff] & 0xff);

            return tmp;
    }
    //TWISTING THE ROAD
    private static byte[] rotateWord(byte[] input) {
            byte[] tmp = new byte[input.length];
            tmp[0] = input[1];
            tmp[1] = input[2];
            tmp[2] = input[3];
            tmp[3] = input[0];

            return tmp;
    }
    
    
       // ROUNDING VALUE
    private static byte[][] AddRoundKey(byte[][] state, byte[][] w, int round) {

            byte[][] tmp = new byte[state.length][state[0].length];

            for (int c = 0; c < factor; c++) {
                    for (int l = 0; l < 4; l++)
                            tmp[l][c] = (byte) (state[l][c] ^ w[round * factor + c][l]);
            }

            return tmp;
    }

    
    // SUBBYTES CALCULATE
    private static byte[][] SubBytes(byte[][] state) {

            byte[][] tmp = new byte[state.length][state[0].length];
            for (int row = 0; row < 4; row++)
                    for (int col = 0; col < factor; col++)
                            tmp[row][col] = (byte) (sbox[(state[row][col] & 0x000000ff)] & 0xff);

            return tmp;
    }
    
    
    
    //iNVERSE THE SUBBYTE
    private static byte[][] InvSubBytes(byte[][] state) {
            for (int row = 0; row < 4; row++) 
                    for (int col = 0; col < factor; col++)
                            state[row][col] = (byte)(inv_sbox[(state[row][col] & 0x000000ff)]&0xff);

            return state;
    }

    
    //SHIFTING THE ROWS
    private static byte[][] ShiftRows(byte[][] state) {

            byte[] t = new byte[4];
            for (int r = 1; r < 4; r++) {
                    for (int c = 0; c < factor; c++)
                            t[c] = state[r][(c + r) % factor];
                    for (int c = 0; c < factor; c++)
                            state[r][c] = t[c];
            }

            return state;
    }

    
    //Inverse of shift row
    private static byte[][] InvShiftRows(byte[][] state) { 
            byte[] t = new byte[4]; 
            for (int r = 1; r < 4; r++) {
                    for (int c = 0; c < factor; c++) 
                            t[(c + r)%factor] = state[r][c];
                    for (int c = 0; c < factor; c++) 
                            state[r][c] = t[c];
            }
    return state;
    }

    
    
    
    
    //Inverse of mix column
    private static byte[][] InvMixColumns(byte[][] s){
             int[] sp = new int[4];
          byte b02 = (byte)0x0e, b03 = (byte)0x0b, b04 = (byte)0x0d, b05 = (byte)0x09;
          for (int c = 0; c < 4; c++) {
             sp[0] = FFMul(b02, s[0][c]) ^ FFMul(b03, s[1][c]) ^ FFMul(b04,s[2][c])  ^ FFMul(b05,s[3][c]);
             sp[1] = FFMul(b05, s[0][c]) ^ FFMul(b02, s[1][c]) ^ FFMul(b03,s[2][c])  ^ FFMul(b04,s[3][c]);
             sp[2] = FFMul(b04, s[0][c]) ^ FFMul(b05, s[1][c]) ^ FFMul(b02,s[2][c])  ^ FFMul(b03,s[3][c]);
             sp[3] = FFMul(b03, s[0][c]) ^ FFMul(b04, s[1][c]) ^ FFMul(b05,s[2][c])  ^ FFMul(b02,s[3][c]);
             for (int i = 0; i < 4; i++) s[i][c] = (byte)(sp[i]);
          }

          return s;
    }

    
    //Mixcolumns calculations
    private static byte[][] MixColumns(byte[][] s){
             int[] sp = new int[4];
          byte b02 = (byte)0x02, b03 = (byte)0x03;
          for (int c = 0; c < 4; c++) {
             sp[0] = FFMul(b02, s[0][c]) ^ FFMul(b03, s[1][c]) ^ s[2][c]  ^ s[3][c];
             sp[1] = s[0][c]  ^ FFMul(b02, s[1][c]) ^ FFMul(b03, s[2][c]) ^ s[3][c];
             sp[2] = s[0][c]  ^ s[1][c]  ^ FFMul(b02, s[2][c]) ^ FFMul(b03, s[3][c]);
             sp[3] = FFMul(b03, s[0][c]) ^ s[1][c]  ^ s[2][c]  ^ FFMul(b02, s[3][c]);
             for (int i = 0; i < 4; i++) s[i][c] = (byte)(sp[i]);
          }

          return s;
    }
       

        //
    public static byte FFMul(byte a, byte b) {
            byte aa = a, bb = b, r = 0, t;
            while (aa != 0) {
                    if ((aa & 1) != 0)
                            r = (byte) (r ^ bb);
                    t = (byte) (bb & 0x80);
                    bb = (byte) (bb << 1);
                    if (t != 0)
                            bb = (byte) (bb ^ 0x1b);
                    aa = (byte) ((aa & 0xff) >> 1);
            }
            return r;
    }
    
    
    
    
    //applying all the functions
    public static byte[] encryptBloc(byte[] in) {
            byte[] tmp = new byte[in.length];



            byte[][] state = new byte[4][factor];

            for (int i = 0; i < in.length; i++)
                    state[i / 4][i % 4] = in[i%4*4+i/4];

            state = AddRoundKey(state, w, 0);
            for (int round = 1; round < factorr; round++) {
                    state = SubBytes(state);
                    state = ShiftRows(state);
                    state = MixColumns(state);
                    state = AddRoundKey(state, w, round);
            }
            state = SubBytes(state);
            state = ShiftRows(state);
            state = AddRoundKey(state, w, factorr);

            for (int i = 0; i < tmp.length; i++)
                    tmp[i%4*4+i/4] = state[i / 4][i%4];

            return tmp;
    }

    
    //Decryption performed
    public static byte[] decryptBloc(byte[] in) {
            byte[] tmp = new byte[in.length];

            byte[][] state = new byte[4][factor];

            for (int i = 0; i < in.length; i++)
                    state[i / 4][i % 4] = in[i%4*4+i/4];

            state = AddRoundKey(state, w, factorr);
            for (int round = factorr-1; round >=1; round--) {
                    state = InvSubBytes(state);
                    state = InvShiftRows(state);
                    state = AddRoundKey(state, w, round);
                    state = InvMixColumns(state);

            }
            state = InvSubBytes(state);
            state = InvShiftRows(state);
            state = AddRoundKey(state, w, 0);

            for (int i = 0; i < tmp.length; i++)
                    tmp[i%4*4+i/4] = state[i / 4][i%4];

            return tmp;
    }

    
    
    //Calculation  of the value encrypted text
    public static byte[] ecb_encrypt(byte[] plainText, byte[] key){

            factor = 4;
            factork = key.length/4;
            factorr = factork + 6;


            int lenght=0;
            byte[] padding = new byte[1];
            int i;
            lenght = 16 - plainText.length % 16;
            padding = new byte[lenght];					
            padding[0] = (byte) 0x80;

            for (i = 1; i < lenght; i++)				
                    padding[i] = 0;

            byte[] ecryptedText = new byte[plainText.length + lenght];
            byte[] bloc = new byte[16];							


            
            // generation of subkeys
            w = generateSubkeys(key);

            
            
            int count = 0;

            for (i = 0; i < plainText.length + lenght; i++) {
                    if (i > 0 && i % 16 == 0) {
                            bloc = encryptBloc(bloc);
                            System.arraycopy(bloc, 0, ecryptedText, i - 16, bloc.length);
                    }
                    if (i < plainText.length)
                            bloc[i % 16] = plainText[i];
                    else{														
                            bloc[i % 16] = padding[count % 16];
                            count++;
                    }
            }
            if(bloc.length == 16){
                    bloc = encryptBloc(bloc);
                    System.arraycopy(bloc, 0, ecryptedText, i - 16, bloc.length);
            }

            return ecryptedText;
    }

    
    
    //decryption of the calculated value
    public static byte[] ecb_decrypt(byte[] ecryptedText,byte[] key){
            int i;
            byte[] plainText = new byte[ecryptedText.length];
            byte[] bloc = new byte[16];


            factor = 4;
            factork = key.length/4;
            factorr = factork + 6;
            w = generateSubkeys(key);


            for (i = 0; i < ecryptedText.length; i++) {
                    if (i > 0 && i % 16 == 0) {
                            bloc = decryptBloc(bloc);
                            System.arraycopy(bloc, 0, plainText, i - 16, bloc.length);
                    }
                    if (i < ecryptedText.length)
                            bloc[i % 16] = ecryptedText[i];
            }
            bloc = decryptBloc(bloc);
            System.arraycopy(bloc, 0, plainText, i - 16, bloc.length);


            plainText = deletePadding(plainText);

            return plainText;
    }

    
    //changes in padding
    private static byte[] deletePadding(byte[] input) {
            int count = 0;

            int i = input.length - 1;
            while (input[i] == 0) {
                    count++;
                    i--;
            }

            byte[] tmp = new byte[input.length - count - 1];
            System.arraycopy(input, 0, tmp, 0, tmp.length);
            return tmp;
    }


    
    //generation of the key and password
    public static String generateKey(int length) {
            Random random = new Random();
            Random random2 = new Random();
            String capitalLetters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
            String smallLetters = "abcdefghijklmnopqrstuvwxyz";
            String numbers = "0123456789";

            String[] allCombinations = { capitalLetters, smallLetters, numbers };// symbols };

            StringBuilder password = new StringBuilder();
            int j, i = 0;

            while (i < length) {
                    password.append(allCombinations[j = (random2.nextInt(allCombinations.length))]
                                    .charAt(random.nextInt(allCombinations[j].length())));
                    i++;
            }

            return password.toString();
    }



}
