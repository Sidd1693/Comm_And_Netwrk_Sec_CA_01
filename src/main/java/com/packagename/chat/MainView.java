...
//publisher and receiver details
...
//getFromOnGUI
    ...
    //getSentOrReceived status o fteh message
...
// getOtherUser is it one of the user that we have set
...
//show user chat with rsa implementation and html format
		...
		messages.subscribe(message -> {
			getUI().ifPresent(ui ->
				ui.access(()->{
					...
					String bodyMessage = ((getSentOrReceived(message).equals("SENT")) ? _body : new String(AES.ecb_decrypt(message.getBody(), Utils.bigIntegerToString(RSA.decrypt(message.getRSAKyEncrypt(),  recievedUser.getD(), recievedUser.getN() )).getBytes())));
...
//creation of component for input layout
